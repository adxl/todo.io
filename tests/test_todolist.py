...
class TodoListTest(TestCase):
    ...
    def test_unique_items(self):
        ...
        self.todolist.set_item(item_1)
        self.assertEqual(self.todolist.get_length(),1)

        self.assertRaises(ValueError,self.todolist.set_item, item_2)
    ...
    def test_add_trigger_email_on_8th_insert(self):
        ...
        for i in range(7):
            self.todolist.set_item(Item(str(i), str(i)))

        self.assertEqual(self.todolist.get_length(), 7)

        with self.assertRaises(NotImplementedError) as context:
            self.todolist.set_item(Item("some_name", "some_content"))
